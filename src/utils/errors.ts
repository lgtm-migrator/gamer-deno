...
export function handleError(message: DiscordenoMessage, type: Errors) {
  switch (type) {
    case Errors.MISSING_SEND_MESSAGES:
      return message.send(missingPermResponse(type)).catch(console.log);
    ...
    case Errors.MISSING_VIEW_AUDIT_LOG:
      return message.send(missingPermResponse(type));
    case Errors.DELETE_MESSAGES_MIN:
      return message.send("You need to provide atleast 2 messages to delete multiple messages at once.");
    ...
    case Errors.NICKNAMES_MAX_LENGTH:
      return message.send("A nickname can not be longer than 32 characters.");
    case Errors.PRUNE_MIN_DAYS:
      return message.send("You can not prune members from the server with less than 1 days activity requirement.");
    ...
    case Errors.MISSING_INTENT_GUILD_MEMBERS:
      return message.send("Unable to fetch members if the bot does not have the GUILD MEMBERS intent.");
    case Errors.BOTS_HIGHEST_ROLE_TOO_LOW:
      return message.send("The bot's highest role is too low to complete this action.");
