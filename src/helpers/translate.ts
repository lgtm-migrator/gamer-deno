...
export function translate(guildID: bigint, key: string, options?: Record<string, unknown>): string;
export function translate(guildID: bigint, key: string, options?: Record<string, unknown>) {
  // SUPPORT LEGACY STRINGS
  if (key === "") return "";

  let language = Gamer.guildLanguages.get(guildID) || Gamer.guilds.get(guildID)?.preferredLocale || "en_US";
...
export function translateArray(guildID: bigint, key: string, options?: Record<string, unknown>): string[] {
  const language = Gamer.guildLanguages.get(guildID) || Gamer.guilds.get(guildID)?.preferredLocale || "en_US";
  ...
  return i18next.use(Backend).init(
    {
      ...
      missingKeyHandler: async function (lng: string, ns: string, key: string, fallbackValue: string) {
        ...
        const channel = Gamer.channels.get(Gamer.transformers.snowflake(configs.channelIDs.missingTranslation));
        if (!channel) return;

        const args = key.split("_");
        if (key.endsWith("_USAGE") && Gamer.commands.has(args[0]?.toLowerCase())) {
          return;
        }

        if (
          ["INVITES", "SERVERS", "PERMS", "MESSAGES", "CHANNELS", "ROLES", "BOTS", "NITRO", "HYPESQUADS"].some(
            (ignore) => key.startsWith(ignore)
          ) &&
          key.endsWith("NOTE")
        ) {
          return;
        ...
        await Gamer.helpers.sendTextMessage(channel.id, response).catch(console.log);
      },
