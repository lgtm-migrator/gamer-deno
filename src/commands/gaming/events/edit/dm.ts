...
createSubcommand("events-edit", {
  ...
  execute: async function (message, args, guild) {
    ...
    const hasPerm =
      (await bot.permissionLevels.get(PermissionLevels.MODERATOR)?.(message, this, guild)) ||
      (await bot.permissionLevels.get(PermissionLevels.ADMIN)?.(message, this, guild));
    ...
    await db.events.update(event.id, { dmReminders: args.enabled });
