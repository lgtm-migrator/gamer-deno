...
createSubcommand("events", {
  ...
  execute: async function (message, args) {
    ...
    const TIME_INFO = [
      translate(message.guildID, "strings:EVENTS_SHOW_TIME_1", {
        value: humanizeMilliseconds(event.duration),
      }),
      translate(message.guildID, "strings:EVENTS_SHOW_TIME_2", {
    ...
    const embed = bot.helpers
      ...
      .addField(translate(message.guildID, `strings:EVENTS_SHOW_TIME_EMOJI`), TIME_INFO.join("\n"))
      .addField(
        ...
        [
          ...
          translate(message.guildID, "strings:EVENTS_SHOW_RSVP_3", {
            value: `${event.acceptedUsers.length} / ${event.maxAttendees}`,
          }),
          translate(message.guildID, "strings:EVENTS_SHOW_RSVP_4", {
            value: attendees.length ? attendees : NONE,
          }),
          translate(message.guildID, "strings:EVENTS_SHOW_RSVP_5", {
            value: maybes.length ? maybes : NONE,
          }),
          translate(message.guildID, "strings:EVENTS_SHOW_RSVP_6", {
            value: waitingList.length ? waitingList : NONE,
          }),
          translate(message.guildID, "strings:EVENTS_SHOW_RSVP_7", {
      ...
      .addField(
        ...
        [
          translate(message.guildID, "strings:EVENTS_SHOW_POSITIONS", {
            value: event.positions.map((p) => `**${p.name}**: ${p.amount}`).join(" ") || NONE,
          }),
          translate(message.guildID, "strings:EVENTS_SHOW_GAMING_1", {
            value: event.platform,
          }),
          translate(message.guildID, "strings:EVENTS_SHOW_GAMING_2", {
            value: event.game,
          }),
          translate(message.guildID, "strings:EVENTS_SHOW_GAMING_3", {
      ...
      .addField(
        ...
        [
          ...
          translate(message.guildID, "strings:EVENTS_SHOW_BASIC_3", {
            value: event.alertRoleIDs.length ? event.alertRoleIDs.map((id) => `<@&${id}>`).join(" ") : NONE,
          }),
        ].join("\n")
      )
