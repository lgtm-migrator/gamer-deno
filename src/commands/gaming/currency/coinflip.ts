...
createCommand({
  ...
  execute: async function (message, args) {
    ...
    if (!args.amount && bot.vipGuildIDs.has(message.guildID)) {
      return message.reply(chooseRandom(["<:heads:787887930534395914>", "<:tails:787887930299514901>"]));
    }

    if (args.amount < 0) return bot.helpers.reactError(message);
    ...
    if (!authorSettings) return bot.helpers.reactError(message);
    ...
    const image =
      coinflip === "heads"
        ? `<:heads:787887930534395914> ${win ? bot.constants.emojis.success : ""}`
        : `<:tails:787887930299514901> ${win ? bot.constants.emojis.success : ""}`;
    ...
    if (win) bot.helpers.reactSuccess(message);
  },
});
